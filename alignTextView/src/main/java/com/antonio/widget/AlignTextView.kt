...
class AlignTextView : TextView {
    /**
     * 当只有一行的时候是否需要对齐，比如登陆场景里面的手机号  密码 两者长度不一致但是需要左右对齐
     */
    var alignOnlyOneLine = false
    ...
    private fun initAttr(context: Context, attributeSet: AttributeSet?) {
        val typedArray: TypedArray = context.obtainStyledAttributes(attributeSet, R.styleable.AlignTextView)
        ...
        if (text !is String) {
            ...
            for (i in 0 until layout.lineCount) {
                ...
                if (alignOnlyOneLine && layout.lineCount == 1) {
                    ...
                    canvas!!.drawText(text.substring(lineStart), paddingLeft.toFloat(), lineBaseline.toFloat(), paint)
                    break
                } else {
                    val line = text.substring(lineStart, lineEnd)
                    val width = StaticLayout.getDesiredWidth(text, lineStart, lineEnd, paint)
